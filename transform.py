...
class CocoLabel(object):
    def __call__(self, inputs):
        # labels = torch.zeros(inputs.size()[0], inputs.size()[1])
        inputs[inputs==255] = 64
        inputs = inputs[:,:,0]/64*100 + inputs[:,:,1]/64*10 + inputs[:,:,2]/64
        inputs[inputs == 0] = 0
        inputs[inputs == 2] = 0
        inputs[inputs == 20] = 0
        inputs[inputs == 10] = 0
